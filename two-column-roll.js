...
function run() {
    ...
    let backgroundSkills = getSkills(actor, 'background');
    ...
    async function roll(actionType){
        ...
        let rank = 0;
        let backgroundSkillInfo = {
            'name':'None',
            'rank':0
        }
        let skillInfo = actor.data.data.skills[skill];
        if (background) {
            backgroundSkillInfo = actor.data.data.skills[background];        
            rank = backgroundSkillInfo.rank + skillInfo.rank
        } else {
            rank = backgroundSkillInfo.rank + skillInfo.rank
        }
        ...
        'content': `<h1>Make a Skill Roll</h1>
            <div style="display:flex; flex-direction: column">         
                <div style="flex:1; display:flex; align-items:center; margin-bottom:10px">
                    <label style="flex:1">Background Skill:</label>
                    <select style="flex:3" id="select-background-skill">
                        ${backgroundSkills.map(s => `<option value="${s.name}">${s.name.replace('[Background]', '')} (rank: ${s.rank})</option>`)}
                    </select>
                </div>
                <div style="flex:1; display:flex; align-items:center; margin-bottom:10px">
